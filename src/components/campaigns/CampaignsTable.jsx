  ...
  return (
    <div>
        ...
        {filterCards.map(card=>{
          ...
          return(
            <div key={card.id} className="ops-fcard"
              ...
              style={{flex:isMobile?"0 0 120px":"1 1 0",minWidth:isMobile?120:100,padding:"14px 16px 12px",borderRadius:10,
                background:active?card.bg:T.bgCard,border:`1px solid ${active?card.color+"55":T.subtle}`,cursor:"pointer",userSelect:"none",
      ...
      <div style={{background:T.bgCard,border:`1px solid ${T.subtle}`,borderRadius:10,overflow:"hidden",boxShadow:"0 2px 12px rgba(0,0,0,0.3)"}}>
        <div style={{padding:"12px 18px",display:"flex",alignItems:"center",justifyContent:"space-between",gap:10,flexWrap:"wrap",borderBottom:`1px solid ${T.subtle}`}}>
        ...
        {loading?(
          <div style={{padding:"56px 20px",textAlign:"center",color:T.muted}}>
            ...
            <p style={{margin:0,fontSize:13,fontFamily:"'DM Sans',sans-serif"}}>Loading campaigns…</p>
          </div>
        ):filtered.length===0?(
          <div style={{padding:"60px 20px",textAlign:"center"}}>
            ...
            <p style={{margin:"8px 0 0",fontSize:13,color:T.muted,fontFamily:"'DM Sans',sans-serif"}}>Try adjusting your search or filter.</p>
          </div>
        ):(
                  ...
                  return(
                    <tr key={c._id} className="ops-row"
                      ...
                      <td style={{padding:"12px 16px",whiteSpace:"nowrap"}}>
                        {creatorName?(
                          <div style={{display:"flex",alignItems:"center",gap:7}}>
                            ...
                            <span style={{fontSize:12,color:T.text,fontFamily:"'DM Sans',sans-serif"}}>{creatorName}</span>
                          </div>
                      ...
                      <td style={{padding:"12px 16px",minWidth:160,maxWidth:260}}>
                        <p style={{margin:0,fontSize:12,color:T.text,lineHeight:1.55,wordBreak:"break-word",whiteSpace:"pre-wrap",fontFamily:"'DM Sans',sans-serif"}}>{c.message}</p>
                      </td>

                      <td style={{padding:"12px 16px",minWidth:130,maxWidth:220}}>
                        {c.pmMessage
                          ?<p style={{margin:0,fontSize:12,color:T.muted,lineHeight:1.55,fontStyle:"italic",wordBreak:"break-word",fontFamily:"'DM Sans',sans-serif"}}>{c.pmMessage}</p>
                          :<span style={{fontSize:11,color:T.subtle,fontFamily:"'JetBrains Mono',monospace"}}>—</span>}
                      </td>
                      ...
                      {/*
                        FIX: Show scheduleAt whenever it exists — not only when action === "approve".
                        This preserves the schedule time after IT marks the campaign as done.
                        Campaigns in transfer/pending state won't have scheduleAt set, so they
                        still correctly show "—".
                      */}
                      <td style={{padding:"12px 16px",whiteSpace:"nowrap"}}>
                        {c.scheduleAt
                      ...
                      <td style={{padding:"12px 16px",minWidth:130,maxWidth:200}}>
                        {c.itMessage
                          ?<span style={{fontSize:11,color:T.teal,fontStyle:"italic",wordBreak:"break-word",display:"block",fontFamily:"'DM Sans',sans-serif"}}>{c.itMessage}</span>
                          :<span style={{fontSize:11,color:T.subtle,fontFamily:"'JetBrains Mono',monospace"}}>—</span>}
                      </td>
