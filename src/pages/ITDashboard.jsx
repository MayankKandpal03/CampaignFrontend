 ...
 *
 * MOBILE LOGIN FIX (see src/api/axios.js)
 *    withCredentials was removed from the global axios defaults. This
 *    prevents CORS preflight failures on mobile carrier networks.
 */
 ...
 * A visible CTA is required because browsers suppress the permission
 * prompt unless it is triggered by a user gesture or clearly surfaced UI.
 * Without this banner, the silent requestNotificationPermission() call in
 * useEffect is ignored and users never see the browser permission dialog,
 * meaning they get zero alerts when they switch tabs.
 */
...
function OverlayNotif({ notif, onAck, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, zIndex:99999, background:"rgba(18,17,12,0.82)", backdropFilter:"blur(6px)", WebkitBackdropFilter:"blur(6px)", display:"flex", alignItems:"center", justifyContent:"center", padding:24, animation:"itOverlayIn 0.3s cubic-bezier(.22,1,.36,1) both" }}>
      <style>{`
      ...
      `}</style>
      <div style={{ width:"100%", maxWidth:480, background:"var(--surface)", border:`1px solid ${isCampaign ? "rgba(42,96,72,0.35)" : "rgba(26,79,110,0.35)"}`, borderRadius:20, overflow:"hidden", boxShadow:"0 32px 80px rgba(0,0,0,0.55)", animation:"itCardIn 0.35s cubic-bezier(.22,1,.36,1) both" }}>
        <div style={{ height:4, background: isCampaign ? "linear-gradient(90deg,var(--accent),#3a7a5a)" : "linear-gradient(90deg,var(--info),#2a6a8e)" }}/>
        <div style={{ padding:"28px 28px 20px", background:"var(--surface2)", borderBottom:"1px solid var(--border)", display:"flex", alignItems:"flex-start", gap:18 }}>
          <div style={{ position:"relative", flexShrink:0 }}>
            <div style={{ position:"absolute", inset:0, borderRadius:"50%", background: isCampaign ? "var(--accent-lt)" : "var(--info-lt)", animation:"itPulseRing 1.6s ease-out infinite" }}/>
            <div style={{ width:52, height:52, borderRadius:"50%", background: isCampaign ? "var(--accent-lt)" : "var(--info-lt)", border:`2px solid ${isCampaign ? "var(--accent)" : "var(--info)"}`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:24, position:"relative", animation:"itBounceIn 0.5s cubic-bezier(.22,1,.36,1) 0.1s both" }}>
              {isCampaign ? "📋" : "🗓"}
            </div>
          </div>
          <div style={{ flex:1, minWidth:0 }}>
            <p style={{ margin:"0 0 5px", fontSize:10, fontFamily:"var(--ff-mono)", letterSpacing:"0.1em", textTransform:"uppercase", color: isCampaign ? "var(--accent)" : "var(--info)", fontWeight:600 }}>
              {isCampaign ? "New Campaign Assigned" : "Daily Task Due"}
            </p>
            <h2 style={{ margin:"0 0 4px", fontSize:20, fontWeight:700, color:"var(--text)", fontFamily:"var(--ff-display)", letterSpacing:"-0.02em", lineHeight:1.2 }}>
              {notif.title}
            </h2>
            <p style={{ margin:0, fontSize:12, color:"var(--muted)", fontFamily:"var(--ff-mono)" }}>
              Received at {new Date().toLocaleTimeString("en-IN", { hour:"2-digit", minute:"2-digit", hour12:true })}
            </p>
          </div>
        </div>
        <div style={{ padding:"20px 28px" }}>
          <div style={{ padding:"14px 16px", background:"var(--surface3)", border:"1px solid var(--border)", borderRadius:10, marginBottom:16 }}>
            <p style={{ margin:"0 0 7px", fontSize:8, fontFamily:"var(--ff-mono)", letterSpacing:"0.1em", textTransform:"uppercase", color:"var(--muted)" }}>
              {isCampaign ? "PM Note / Message" : "Task Description"}
            </p>
            <p style={{ margin:0, fontSize:14, color:"var(--ink)", lineHeight:1.65, wordBreak:"break-word", fontFamily:"var(--ff-body)" }}>
              {notif.body || "—"}
            </p>
          </div>
          {isCampaign && notif.item?.scheduleAt && (
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", background:"var(--accent-lt)", borderRadius:8, marginBottom:16 }}>
          ...
          {!isCampaign && notif.item?.time && (
            <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", background:"var(--info-lt)", borderRadius:8, marginBottom:16 }}>
              <span style={{ fontSize:14 }}>⏰</span>
              <div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:8, padding:"10px 14px", background:"rgba(143,66,12,0.08)", border:"1px solid rgba(143,66,12,0.2)", borderRadius:8, marginBottom:20 }}>
          ...
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={onClose}
              style={{ flex:1, padding:"12px", borderRadius:10, border:"1.5px solid var(--border2)", background:"none", fontFamily:"var(--ff-body)", fontSize:13, fontWeight:500, color:"var(--muted)", cursor:"pointer", transition:"all 0.15s ease" }}
            ...
            <button onClick={() => onAck(notif)}
              style={{ flex:2, padding:"12px", borderRadius:10, border:"1.5px solid transparent", background: isCampaign ? "linear-gradient(135deg,var(--accent),#3a7a5a)" : "linear-gradient(135deg,var(--info),#2a6a8e)", fontFamily:"var(--ff-body)", fontSize:14, fontWeight:600, color:"#fff", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", gap:8, transition:"all 0.15s ease" }}
              onMouseEnter={e => { e.currentTarget.style.opacity="0.9"; e.currentTarget.style.transform="translateY(-1px)"; }}
              onMouseLeave={e => { e.currentTarget.style.opacity="1"; e.currentTarget.style.transform="none"; }}>
              <span>✓</span> Acknowledge Now
            </button>
          </div>
        </div>
        <div style={{ padding:"12px 28px", background:"var(--surface2)", borderTop:"1px solid var(--border)", display:"flex", alignItems:"center", gap:6 }}>
...
export default function ITDashboard() {
  ...
  // Tracked as state so the banner re-renders when the user grants permission.
  const [notifPermission, setNotifPermission] = useState(() =>
    getNotificationPermission()
  );

  // ── BroadcastChannel for cross-tab sync ───────────────────────────────────
  // When this tab's socket fires a notification, broadcast to other IT
  // dashboard tabs in the same browser so they also show the overlay.
  ...
  useEffect(() => {
    ...
    bc.onmessage = ({ data }) => {
      ...
      // Avoid duplicating an overlay that this tab already queued
      setOverlayQueue(prev =>
        prev.some(n => n.id === notif.id) ? prev : [...prev, notif]
      );
      // Also play sound on the receiving tab
  ...
  // ── The topmost overlay is the one currently displayed ────────────────────
  const currentOverlay = overlayQueue[0] ?? null;

  // ── Notification permission: request on banner click (user gesture) ───────
  const handleRequestPermission = useCallback(async () => {
    const granted = await requestNotificationPermission();
    setNotifPermission(granted ? "granted" : "denied");
  }, []);

  // ── Push a new overlay and broadcast to sibling tabs ─────────────────────
  const pushOverlay = useCallback((notif) => {
    const entry = { ...notif, id: Date.now() + Math.random() };

    // Native OS notification — visible above ALL apps and tabs.
    // Returns true only when permission is "granted"; otherwise the user
    // sees only the in-app overlay (and the permission banner).
    triggerAlert(
      entry.title,
      entry.body,
    ...
    // Broadcast to other IT dashboard tabs in this browser
  ...
  return (
    <div className="it-root" style={{ height:"100vh", overflow:"hidden" }}>
      ...
      <div className="it-main" style={{ display:"flex", flexDirection:"column", overflow:"hidden" }}>
        ...
        {/* ── Notification permission banner (KEY FIX: visible CTA so
              the browser actually shows its permission prompt) ── */}
        ...
        <header className="it-header">
          ...
          <div style={{ position:"relative" }}>
            <div className="notif-bell" title={`${unread} unread notifications`} onClick={() => pushToast(`You have ${unread} notification(s)`)}>
              🔔
              {unread > 0 && <span className="notif-dot"/>}
            </div>
            {overlayQueue.length > 0 && (
              <span style={{ position:"absolute", top:-6, right:-6, minWidth:18, height:18, borderRadius:99, background:"var(--danger)", color:"#fff", fontSize:10, fontFamily:"var(--ff-mono)", fontWeight:700, display:"flex", alignItems:"center", justifyContent:"center", padding:"0 4px", border:"2px solid var(--bg)" }}>
                {overlayQueue.length}
              </span>
            )}
          </div>
      ...
      {currentOverlay && (
        <OverlayNotif notif={currentOverlay} onClose={dismissOverlay} onAck={ackFromOverlay}/>
      )}
