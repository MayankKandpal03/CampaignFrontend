/**
 * ManagerDashboard — premium redesign.
...
import { fmt, initials, toLocalISO, localToUTC } from "../utils/formatters.js"; // add localToUTC
import {
  fetchCampaigns,
...
export default function ManagerDashboard() {
  ...
  const [createForm, setCreateForm] = useState({
    message: "",
    requestedAt: toLocalISO(new Date()),
  });
    ...
    if (searchQuery.trim()) {
      ...
      list = list.filter(
        (c) =>
          ...
          (typeof c.createdBy === "object" ? c.createdBy?.username : "")
            ?.toLowerCase()
            .includes(q),
      );
    }
  ...
  const handleCreate = useCallback(
    async (e) => {
      ...
      setCreateOk(false);
      if (!teamId) {
        setCreateError("Team not found. Please wait or refresh.");
        return;
      }
      ...
      setCreating(true);
      try {
        const newC = await createCampaignService({
          message: createForm.message.trim(),
          requestedAt: localToUTC(createForm.requestedAt) || undefined, // ← FIX
          teamId,
        });
        if (newC)
          setCampaigns((prev) => {
            if (prev.some((x) => x._id === newC._id)) return prev;
            return [newC, ...prev];
          });
        setCreateForm({ message: "", requestedAt: toLocalISO(new Date()) });
        setCreateOk(true);
            ...
            >
              ...
              {camLoading ? (
                ...
                >
                  <div
                    style={{
                      ...
                      margin: "0 auto 14px",
                      animation: "opsSpinner .8s linear infinite",
                    }}
                  />
                ...
                <EmptyState
                  ...
                  action={
                    !searchQuery && !statusFilter ? (
                      <GoldBtn variant="outline" onClick={() => goTo("create")}>
                        Create Campaign
                      </GoldBtn>
                    ) : null
                  }
                      ...
                      {filtered.map((c, i) => {
                        ...
                        const creatorId =
                          typeof c.createdBy === "object"
                            ? c.createdBy?._id
                            : c.createdBy;
                        const isOwn = !teamInfo?.members?.find(
                          (m) => String(m._id) === String(creatorId),
                        );
...
// Allow edit if not yet approved, OR approved but schedule hasn't fired yet
  ...
  : c.status === "not done" ? T.amber : T.green;
              ...
              {!camLoading && filtered.length > 0 && (
                <div
                  style={{
                    padding: "9px 18px",
                    borderTop: `1px solid ${T.subtle}22`,
                    display: "flex",
            ...
            <div style={{ maxWidth: 560 }}>
              <div
                style={{
                  ...
                  marginBottom: 20,
                  background: T.bgCard,
                  border: `1px solid ${teamId ? T.subtle : T.red + "44"}`,
                  borderRadius: 8,
                }}
              >
                <span
                  style={{
                    ...
                    boxShadow: teamId ? `0 0 8px ${T.green}` : "none",
                  }}
                />
                <div>
                  <p
                  ...
                  <p
                    style={{
                      margin: "2px 0 0",
                      fontSize: 11,
                      color: teamId ? T.gold : T.red,
                      fontFamily: "'JetBrains Mono',monospace",
                    }}
                  >
                    {teamId
                      ? `${String(teamId).slice(0, 24)}…`
