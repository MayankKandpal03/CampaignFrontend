...
export default function PMDashboard() {
  ...
  const InfoBar = ({ color, message }) => (
    <div
      className="flex gap-3 items-center mb-5 px-4.5 py-3 bg-white rounded-lg shrink-0"
    ...
    >
      ...
      />
      <p className="m-0 text-xs text-[#8a8475] leading-[1.6]">{message}</p>
    </div>
  );
    ...
    <div className="flex h-screen overflow-hidden bg-[#faf8f4] text-[#2d2a24] font-['DM_Sans',sans-serif]">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto overflow-x-hidden">
        ...
        {notifPermission !== "granted" && notifPermission !== "unsupported" && (
          <div className={`px-7 py-2.5 flex justify-between items-center gap-3 flex-wrap border-b ${notifPermission === "denied" ? "bg-[rgba(184,48,48,0.06)] border-[rgba(184,48,48,0.15)]" : "bg-[rgba(143,66,12,0.06)] border-[rgba(143,66,12,0.12)]"}`}>
            <div className="flex items-center gap-2.25">
              <span className="text-[15px]">{notifPermission === "denied" ? "🔕" : "🔔"}</span>
              <div>
                <p className={`m-0 text-xs font-medium ${notifPermission === "denied" ? "text-[#b83030]" : "text-[#8f420c]"}`}>
            ...
            {notifPermission !== "denied" && (
              <button
                ...
                className="px-4 py-1.5 rounded border border-[#8f420c] bg-transparent text-[#8f420c] text-[11px] font-semibold cursor-pointer font-['Fraunces',serif] tracking-widest whitespace-nowrap transition-all duration-150 hover:bg-[#8f420c] hover:text-white shrink-0"
              >
                ENABLE
              </button>
            )}
        ...
        {activeSection === "manage-users" && (
          <div className={`${isMobile ? "px-3.5 py-4" : "px-7 py-5.5"} flex-1 overflow-y-auto`}>
            <div className="max-w-135">
              ...
              <div className={`bg-white border border-[#e8e5de] rounded-[10px] shadow-[0_1px_3px_rgba(0,0,0,0.06)] ${isMobile ? "p-[22px_18px]" : "p-[28px_28px_24px]"}`}>
                <p className="m-0 mb-1 text-[8px] tracking-[0.22em] text-[rgba(42,96,72,0.6)] font-['Fraunces',serif] uppercase">New Account</p>
                <h2 className="m-0 mb-5.5 text-lg font-semibold text-[#1a1810] font-['Fraunces',serif]">Create User Account</h2>
          ...
          <div className={`${isMobile ? "px-3.5 py-4" : "px-7 py-5.5"} flex-1 overflow-y-auto`}>
            ...
            <div className={`grid gap-6 items-start ${isMobile ? "grid-cols-1" : "grid-cols-2"}`}>
              ...
              <div className="bg-white border border-[#e8e5de] rounded-[10px] p-[24px_22px] shadow-[0_1px_3px_rgba(0,0,0,0.06)]">
                <p className="m-0 mb-1 text-[8px] tracking-[0.22em] text-[rgba(42,96,72,0.6)] font-['Fraunces',serif] uppercase">New Schedule</p>
                <h2 className="m-0 mb-5 text-base font-semibold text-[#1a1810] font-['Fraunces',serif]">Create Daily Task</h2>
                {taskError && (
                  <div className="px-3.5 py-2.5 rounded-lg mb-4 bg-[rgba(184,48,48,0.06)] border border-[rgba(184,48,48,0.15)] text-[#b83030] text-xs">{taskError}</div>
                )}
                {taskOk && (
                ...
                <form onSubmit={handleCreateTask}>
                  ...
                  <Field label="Daily Delivery Time" hint="24-hour IST — sent to IT every day at this time">
                    <input
                      type="time"
                      className={`${INPUT_CLS} rounded-lg`}
              ...
              <div>
                <div className="flex justify-between items-center mb-3.5">
                  <p className="m-0 text-[8px] text-[#8a8475] tracking-[0.2em] font-['Fraunces',serif] uppercase">Scheduled Tasks</p>
                  <div className="flex gap-2 items-center">
                    <span className="px-2.25 py-0.5 rounded-full bg-[rgba(107,79,160,0.08)] border border-[rgba(107,79,160,0.15)] text-[9px] text-[#6b4fa0] font-['JetBrains_Mono',monospace] font-semibold">
                ...
                {taskLoading ? (
                  ...
                  <div className="py-10 px-5 text-center bg-white border border-[#e8e5de] rounded-[10px]">
                    <p className="m-0 text-[13px] text-[#1a1810] font-['Fraunces',serif]">No Daily Tasks Yet</p>
                    <p className="m-0 mt-1.5 text-xs text-[#8a8475]">Create your first daily task using the form.</p>
                  </div>
                ) : (
                  <div className="flex flex-col gap-2.5">
                    {dailyTasks.map(task => {
                      ...
                      return (
                        <div
                          key={task._id}
                          className={`bg-white rounded-lg p-[14px_16px] border-l-[3px] ${isActive ? "opacity-100" : "opacity-55"}`}
                          style={{
                        ...
                        >
                          <div className="flex items-start justify-between gap-2 mb-2.5">
                            <p className="m-0 text-[13px] text-[#2d2a24] leading-[1.55] flex-1 wrap-break-word">{task.task}</p>
                            {isActive && (
                              <button
                                onClick={() => handleDeactivateTask(task._id)}
                                disabled={deactivatingId === task._id}
                                className={`px-2.5 py-0.75 rounded bg-[rgba(184,48,48,0.06)] border border-[rgba(184,48,48,0.15)] text-[#b83030] text-[9px] font-bold tracking-widest cursor-pointer font-['Fraunces',serif] shrink-0 ${deactivatingId === task._id ? "opacity-50" : "opacity-100"}`}
                              >
                          ...
                          <div className="flex items-center gap-3.5 flex-wrap">
                            <span className="flex items-center gap-1.25 text-[11px] text-[#6b4fa0] font-['JetBrains_Mono',monospace]">🕐 {task.time} daily</span>
                            <span className="text-[11px] text-[#8a8475] font-['JetBrains_Mono',monospace]">{ackCount} acknowledgement{ackCount !== 1 ? "s" : ""}</span>
                            <span className={`px-2 py-0.5 rounded-full text-[9px] font-bold tracking-widest font-['Fraunces',serif] ${isActive ? "bg-[rgba(107,79,160,0.08)] text-[#6b4fa0] border border-[rgba(107,79,160,0.15)]" : "bg-[#e8e5de] text-[#8a8475] border border-transparent"}`}>
                          ...
                          {ackCount > 0 && (() => {
                            ...
                            return (
                              <div className="mt-2.5 p-[8px_12px] bg-[#f8f7f4] rounded-md border border-[#e8e5de]">
                                <p className="m-0 mb-0.75 text-[8px] text-[#8a8475] tracking-[0.12em] font-['Fraunces',serif] uppercase">Latest IT Response</p>
                                <p className="m-0 text-[11px] text-[#1a6040] leading-normal whitespace-pre-wrap wrap-break-word">{latest.message}</p>
                                <p className="m-0 mt-1 text-[9px] text-[#8a8475] font-['JetBrains_Mono',monospace]">{fmt(latest.acknowledgedAt)}</p>
                              </div>
                            );
                          })()}
                        </div>
