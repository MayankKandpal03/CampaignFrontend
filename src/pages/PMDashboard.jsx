// src/pages/PMDashboard.jsx — premium redesign
// All existing logic unchanged. Daily Tasks section added.
...
export default function PMDashboard() {
  ...
  useSocket({
    "campaign:created":  c => {
      setCampaigns(p => { if(p.some(x=>x._id===c._id)) return p; return [c,...p]; });
    ...
    // ── Daily task socket events ─────────────────────────────────────────────
    "dailytask:acked": (t) => {
      // Update response count in PM's task list
      setDailyTasks(prev =>
        prev.map(task =>
          task._id === t._id
            ? { ...task, itResponse: t.itResponse }
            : task
        )
      );
  ...
  // FIX: "not done" is now also a closed state
  const closedRequests = useMemo(
    () => campaigns.filter(c=>
      c.status==="cancel"||c.action==="cancel"||c.status==="done"||c.status==="not done"
    ),
    [campaigns]
  );
  ...
  const activeTasks = useMemo(
    () => dailyTasks.filter(t => t.isSchedule !== false),
    [dailyTasks]
  );
  ...
  const InfoBar = ({ color, message }) => (
    <div style={{ display:"flex", gap:12, alignItems:"center", marginBottom:20, padding:"12px 18px", background:T.bgCard, border:`1px solid ${color}25`, borderRadius:8 }}>
      <span style={{ width:7, height:7, borderRadius:"50%", background:color, flexShrink:0, boxShadow:`0 0 8px ${color}` }}/>
      <p style={{ margin:0, fontSize:12, color:T.muted, lineHeight:1.6, fontFamily:"'DM Sans',sans-serif" }}>{message}</p>
    </div>
  );
      ...
      <main style={{ flex:1, display:"flex", flexDirection:"column", minWidth:0 }}>
        ...
        <div style={{ padding:pad, flex:1 }}>
          ...
          {activeSection==="campaigns" && (
            <div style={{ animation:"opsFadeUp .25s ease" }}>
          ...
          {activeSection==="manage-users" && (
            <div style={{ animation:"opsFadeUp .25s ease", maxWidth:540 }}>
          ...
          {/* ── CLOSED REQUESTS (now includes not done) ── */}
          ...
          {activeSection==="daily-tasks" && (
            <div style={{ animation:"opsFadeUp .25s ease" }}>
              <InfoBar color={T.purple}
                message="Schedule recurring daily tasks for IT. Each task is automatically delivered to IT at the specified time every day. IT will acknowledge it daily."/>
              ...
              <div style={{ display:"grid", gridTemplateColumns:isMobile?"1fr":"1fr 1fr", gap:24, alignItems:"start" }}>
                ...
                <div style={{ background:T.bgCard, border:`1px solid ${T.subtle}`, borderRadius:10, padding:"24px 22px", boxShadow:"0 2px 12px rgba(0,0,0,0.3)" }}>
                  ...
                  {taskError && (
                    <div style={{ padding:"10px 14px", borderRadius:8, marginBottom:16, background:T.redBg, border:`1px solid ${T.red}44`, color:T.red, fontSize:12 }}>
                      {taskError}
                    </div>
                  )}
                  {taskOk && (
                  ...
                  <form onSubmit={handleCreateTask}>
                    ...
                    <Field label="Daily Delivery Time" hint="24-hour format — task sent to IT at this time every day">
                      <input
                        type="time"
                        className="ops-focus"
                        value={taskForm.time}
                ...
                <div>
                  <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:14 }}>
                    ...
                    <div style={{ display:"flex", gap:8, alignItems:"center" }}>
                      ...
                      <GoldBtn variant="outline" onClick={loadDailyTasks} style={{ padding:"5px 12px", fontSize:9 }}>
                        Refresh
                      </GoldBtn>
                    </div>
                  </div>
                        ...
                        return (
                          <div
                            ...
                            style={{
                              ...
                              borderLeft:   `3px solid ${isActive ? T.purple : T.muted}`,
                              borderRadius: 8,
                              padding:      "14px 16px",
                          ...
                          >
                            ...
                            <div style={{ display:"flex", alignItems:"flex-start", justifyContent:"space-between", gap:8, marginBottom:10 }}>
                              <p style={{ margin:0, fontSize:13, color:T.text, lineHeight:1.55, flex:1, wordBreak:"break-word", fontFamily:"'DM Sans',sans-serif" }}>
                                {task.task}
                              </p>
                              {isActive && (
                                <button
                                  ...
                                  style={{
                                    ...
                                    flexShrink:  0,
                                    opacity:     deactivatingId === task._id ? 0.5 : 1,
                                  }}
                                >
                            ...
                            <div style={{ display:"flex", alignItems:"center", gap:14, flexWrap:"wrap" }}>
                              <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:11, color:T.purple, fontFamily:"'JetBrains Mono',monospace" }}>
                                🕐 {task.time} daily
                              </span>
                              ...
                              <span style={{
                                ...
                                letterSpacing:"0.1em",
                                fontFamily:   "'Cinzel',serif",
                                background:   isActive ? T.purpleBg : T.subtle,
                            ...
                            {/* Latest acknowledgement preview */}
                            {ackCount > 0 && (() => {
                              ...
                              return (
                                <div style={{ marginTop:10, padding:"8px 12px", background:T.bgInput, borderRadius:6, border:`1px solid ${T.subtle}` }}>
                                  <p style={{ margin:"0 0 3px", fontSize:8, color:T.muted, letterSpacing:"0.12em", fontFamily:"'Cinzel',serif", textTransform:"uppercase" }}>
                                    Latest IT Response
                                  </p>
                                  <p style={{ margin:0, fontSize:11, color:T.teal, lineHeight:1.5, whiteSpace:"pre-wrap", wordBreak:"break-word", fontFamily:"'DM Sans',sans-serif" }}>
                                    {latest.message}
                                  </p>
                                  <p style={{ margin:"4px 0 0", fontSize:9, color:T.muted, fontFamily:"'JetBrains Mono',monospace" }}>
                                    {fmt(latest.acknowledgedAt)}
                                  </p>
                                </div>
                              );
                            })()}
